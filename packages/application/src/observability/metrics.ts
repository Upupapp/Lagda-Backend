...
export const METRIC_NAMES = [
  ...
  "email_delivery_results_total",
  "email_delivery_duration_ms",
  "email_delivery_retries_total",
  "email_delivery_ambiguous_total",
  "email_provider_webhook_events_total",
  "email_dispatch_sweeps_total",
] as const;
...
export const METRIC_LABELS = {
  ...
  // log. Also NOT the documentId, artifactId, slotId or fieldId any of these
  // introduced — all are unbounded, the same reason `document_operations_total`
  // excludes them.
  ...
  // `result` only. NOT the decline reason: it is the recipient's own statement
  // about a document, and a metric label is the most widely replicated string
  ...
  // the destination address (S210), the provider message reference (S211), the
  // notification intent or delivery id, the workspace, the user, the subject,
  // or any part of a rendered body. The first is personal data belonging to a
  // counterparty who is not a LAGDA user; the second and third are unbounded
  // and would produce one time series per message; the rest are content.
  //
  // `notificationType` is a five-value union and is the one dimension worth
  // having: "are password resets failing while signing invitations are fine"
  // is the question an operator actually asks at 3am.
  email_delivery_attempts_total: ["notificationType", "provider", "processRole"],
  email_delivery_results_total:
    ["result", "notificationType", "provider", "processRole"],
  email_delivery_duration_ms: ["result", "provider", "processRole"],
  email_delivery_retries_total: ["notificationType", "provider", "processRole"],
  // Its own counter rather than a `result` value on the one above, because it
  // is the number that decides whether the ambiguity policy is costing
  // duplicates — and a value buried in a shared counter is a value nobody
  // alerts on (S50, OD-176).
  email_delivery_ambiguous_total: ["notificationType", "provider", "processRole"],
  // `eventType` is DELIVERED or BOUNCED and nothing else; `result` records
  // whether the confirmed event actually moved a delivery. Deliberately NOT the
  // provider message reference, which is what a webhook most obviously carries
  // and least safely labels.
  email_provider_webhook_events_total:
    ["eventType", "result", "provider", "processRole"],
  email_dispatch_sweeps_total: ["result", "processRole"],
