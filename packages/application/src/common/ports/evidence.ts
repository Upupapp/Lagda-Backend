...
/**
 * OD-135. Whether the completed document a verification ID names has a
 * participant at this email address, in ANY role — the caller's decision,
 * carried in this projection's absence of a role filter.
 *
 * Same "one answer for absent and restricted" rule as
 * `PublicVerificationProjection`: `null` means "no completed record at that
 * ID" AND "no participant there with that email," indistinguishably. An
 * anonymous caller must learn neither which IDs exist nor which addresses
 * are on one they already hold.
 */
...
/**
 * The sealed bytes' location, once a participant match has already been
 * proven. Never handed out on the strength of the verification ID alone.
 */
export interface PublicVerificationDocumentRef {
...
/**
 * The email-gated view (OD-135). A separate interface from
 * `PublicVerificationLookup` because it takes a SECOND input — the email —
 * that the plain lookup never sees, and because both of its methods
 * re-derive "is this a completed record" from scratch rather than trusting a
 * prior call.
 */
export interface PublicParticipantLookup {
  findMatch(
    verificationId: VerificationId,
    normalizedEmail: string,
  ): Promise<PublicParticipantMatch | null>;

  /** Re-verifies the match; never trusts an earlier `findMatch` call. */
  findDocumentRef(
    verificationId: VerificationId,
    normalizedEmail: string,
  ): Promise<PublicVerificationDocumentRef | null>;
}
