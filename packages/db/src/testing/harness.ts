...
export async function truncateAll(database: LagdaDatabase): Promise<void> {
  ...
  // 082. No runtime DELETE; the harness owns the table.
  await database.db.deleteFrom("workspace_branding").execute();
  ...
  await database.db.deleteFrom("final_copy_grants").execute();
