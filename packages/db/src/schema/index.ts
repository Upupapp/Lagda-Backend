 ...
 * read" / "not dismissed". `evidence_event_id` is foreign-keyed to a real
 * event in the same workspace, so a row cannot name an event that does not
 * exist. No DELETE is granted: clearing a state nulls its timestamp.
 */
