...
import { createTransactionManager, unwrapTransaction } from "./transactions/index.js";
import {
...
suite("workspace tenancy (RLS, runtime role)", () => {
  ...
  beforeEach(async () => {
    ...
    const workspaces = createWorkspaceRepository(owner.db);
    const memberships = createWorkspaceMembershipRepository(owner.db);

    for (const [id, member] of [[WS_A, "mem_a"], [WS_B, "mem_b"]] as const) {
      await tx.runForWorkspace(id, async t => {
        await workspaces.save(
          { workspaceId: id, name: `Workspace ${id}`, ownerUserId: USER, createdAt: AT }, t,
        );
  ...
  describe("reads", () => {
    it("sees its own workspace's members", async () => {
      ...
      const rows = await tx.runForWorkspace(WS_A, t =>
        createWorkspaceMembershipRepository(app.db).listForWorkspace(WS_A, t));
    ...
    it("CANNOT see another workspace's members, even asking for them by ID", async () => {
      ...
      const tx = createTransactionManager(app.db);
      const found = await tx.runForWorkspace(WS_A, t =>
        createWorkspaceMembershipRepository(app.db)
          .findInWorkspace(WS_B, "mem_b" as WorkspaceMemberId, t));
    ...
    it("CANNOT see another workspace's rows even with NO predicate at all", async () => {
      ...
      const tx = createTransactionManager(app.db);
      const rows = await tx.runForWorkspace(WS_A, t =>
        unwrapTransaction(t).selectFrom("workspace_memberships").selectAll().execute());
      expect(rows).toHaveLength(1);
  ...
  describe("writes", () => {
    it("CANNOT insert a row belonging to another workspace", async () => {
      ...
      const tx = createTransactionManager(app.db);
      const failure = await tx.runForWorkspace(WS_A, t =>
        unwrapTransaction(t).insertInto("workspace_memberships").values({
    ...
    it("CANNOT move its own row into another workspace", async () => {
      ...
      const tx = createTransactionManager(app.db);
      const failure = await tx.runForWorkspace(WS_A, t =>
        unwrapTransaction(t).updateTable("workspace_memberships")
    ...
    it("CANNOT update another workspace's row", async () => {
      const tx = createTransactionManager(app.db);
      const result = await tx.runForWorkspace(WS_A, t =>
        unwrapTransaction(t).updateTable("workspace_memberships")
          .set({ role: "auditor" })
    ...
    it("CANNOT delete another workspace's row", async () => {
      const tx = createTransactionManager(app.db);
      const result = await tx.runForWorkspace(WS_A, t =>
        unwrapTransaction(t).deleteFrom("workspace_memberships")
          .where("member_id", "=", "mem_b")
          .execute());
  ...
  describe("missing tenant context", () => {
    it("sees NOTHING rather than everything", async () => {
      ...
      // cross-tenant read.
      const tx = createTransactionManager(app.db);
      const rows = await tx.runGlobal(t =>
        unwrapTransaction(t).selectFrom("workspace_memberships").selectAll().execute());
    ...
    it("cannot insert tenant rows without context", async () => {
      const tx = createTransactionManager(app.db);
      const failure = await tx.runGlobal(t =>
        unwrapTransaction(t).insertInto("workspaces").values({
  ...
  describe("context does not leak between transactions", () => {
    ...
    it("survives many alternating transactions on a shared pool", async () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        const rows = await tx.runForWorkspace(target, t =>
          unwrapTransaction(t).selectFrom("workspace_memberships").selectAll().execute());
  ...
  describe("cross-workspace relationships", () => {
    it("CANNOT attach a member to a workspace it does not belong to", async () => {
      ...
      // BACKEND-08 onward.
      const tx = createTransactionManager(app.db);
      const failure = await tx.runForWorkspace("ws_ghost" as WorkspaceId, t =>
        unwrapTransaction(t).insertInto("workspace_memberships").values({
