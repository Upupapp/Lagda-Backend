...
import type { TransactionId, UserId, WorkspaceId, WorkspaceMemberId } from "@lagda/contracts";
import type { EvidenceEventId, WorkspaceUnitOfWork } from "@lagda/application";
    ...
    for (const [id, member] of [[WS_A, "mem_dns_a"], [WS_B, "mem_dns_b"]] as const) {
      await tx.runForWorkspace(id, async uow => {
        ...
        await uow.evidence.append({
          ...
          signingRequestId: `sreq_${id}` as TransactionId,
