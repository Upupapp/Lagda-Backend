...
// (user, event). The repository writes with INSERT ... SELECT FROM
// evidence_events, so a row can only name an event that exists in the same
// workspace, and a client posting invented ids cannot grow this table.
//
// ── Deliberately NO foreign key to evidence_events ─────────────────────────
//
// A foreign key takes a FOR KEY SHARE lock on the referenced row, and
// PostgreSQL requires UPDATE privilege for any row lock. `evidence_events` is
// append-only: in production it is owned by `lagda_app` with UPDATE revoked,
// so an FK here would make EVERY state write fail with "permission denied" —
// while passing every local test, where the owner is a superuser. That is
// exactly how 059 once broke production; the 065 guard in
// workflow-templates.integration.test.ts asserts the rule, and caught this
// one before it shipped. The SELECT above needs no lock, so it validates
// without the trap.
//
...
export async function up(db: Kysely<unknown>): Promise<void> {
  await sql`
    create table document_notification_states (
      ...
      -- No FK to evidence_events — see the header.
    )
  `.execute(db);
